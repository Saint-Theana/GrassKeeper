...
import org.sorapointa.proto.PlayerChatNotify;

public class PlayerChatNotifyHandler extends BaseHandler
{

	public PlayerChatNotifyHandler(GameClient gameClient, User user, ClientConfig clientConfig)
	{
	...
	{
		PlayerChatNotify notify= ProtobufDecoder.decodeFrom(new PlayerChatNotify(),payload);
		...
		{
			message=info.text;
			gameClient.getRobot().onChannelMessage(notify.channelId,info.uid,info.text);
		}
		else if (info.icon != null)
		{
			message="icon:["+info.icon+"]";
			gameClient.getRobot().onChannelMessage(notify.channelId,info.uid,info.icon);
		}
		System.out.println("recieve message in channel "+notify.channelId+" from "+info.uid+ " message: "+message);
