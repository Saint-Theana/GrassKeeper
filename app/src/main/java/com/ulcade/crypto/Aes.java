...
public class Aes
{
	...
	private static void xorr(byte[] a, byte[] b) {
		int n = 16; // 16 rounds
		int i = 0;
		while (i < n) {
	...
	private static void xorRoundKey(byte[] state, byte[] keys,int round) {
		xorr(state, copyOfRange(keys,round * 16, 16));
	}

	private static byte[] copyOfRange(byte[] keys, int pos, int size)
	{
		byte[] a=new byte[size];
		System.arraycopy(keys,pos,a,0,size);
		return a;
	}
	...
	private static void shiftRowsInv(byte[] state) {
		...
		for (int i=0;i<16;i++) {
			state[i & 0xFF] = temp[shiftRowsTableInv[i & 0xFF] & 0xFF];
		}
	}


	private static void mixColInv(byte[] state, int start) {
		int a0, a1, a2, a3;
		a0=state[start] &0xFF;
		a1=state[start+1] &0xFF;
		a2=state[start+2] &0xFF;
		a3=state[start+3] &0xFF;
		state[start] = (byte)(lookupG14[a0] ^ lookupG9[a3] ^ lookupG13[a2] ^ lookupG11[a1]);
		state[start+ 1] = (byte)(lookupG14[a1] ^ lookupG9[a0] ^ lookupG13[a3] ^ lookupG11[a2]);
		state[start+ 2] = (byte)(lookupG14[a2] ^ lookupG9[a1] ^ lookupG13[a0] ^ lookupG11[a3]);
		state[start+ 3] = (byte)(lookupG14[a3] ^ lookupG9[a2] ^ lookupG13[a1] ^ lookupG11[a0]);
	}
	...
	public static void oqs128Encode(byte[] plainText, byte[] schedule, byte[] cipherText) {
